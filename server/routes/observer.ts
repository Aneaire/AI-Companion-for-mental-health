...
// Observer logging function - easily removable
const logObserverCall = async (
  ...
  try {
    // Create observer_logs directory if it doesn't exist
    const logsDir = path.join(process.cwd(), "observer_logs");
    ...
    const logContent = `# Observer Call Log - ${dateStr} at ${timeStr}
...
---
*Generated by Observer API at ${now.toISOString()}*
`;

    // Write to file
    await fs.promises.writeFile(filepath, logContent, "utf8");
  } catch (error) {
    console.error("❌ Error saving observer log:", error);
    console.error(
      "❌ Error details:",
