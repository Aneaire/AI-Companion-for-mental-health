...
export const Sidebar = memo(function Sidebar({
  ...
  // Infinite scroll: load next page when inView
  useEffect(() => {
    if (inView && setOffset && threads.length > 0 && offset + limit < total) {
      setOffset(offset + limit);
    }
  }, [inView, setOffset, offset, limit, total, threads.length]);
  ...
  return (
    <>
      ...
      <aside
        ref={sidebarRef}
        id="sidebar"
        className={`fixed md:static inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 flex flex-col h-full shadow-sm transform transition-transform duration-200 ease-in-out md:translate-x-0 ${
                          ...
                          {sessions.map((session) => (
                            <li key={session.id}>
                              <button
                                ...
                                // Allow selecting finished sessions to view history
                                title={
                                  session.sessionName ||
